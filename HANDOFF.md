...
- **音符の順番入力**（GridEditor + theory/gridEntry.ts）:
  ...
  - 入力開始は1小節目。全体譜は小節選択、拡大譜面は音符選択で修正・休符選択で入力場所を移動。点のセル列は使わず、オレンジの縦線と「戻る／進む」で現在地を示す。移動先は音価に応じた細分位置と既存音の開始・終了で、音の途中は飛ばす。
  - 譜面は同時に1つだけ。GridEditor内の「小節を編集 / 全体を見る」で出し分ける。GridComposer・EditableStepBodyに別の全体譜を置かない。表示切替でGridEditor自体をアンマウントせず、カーソル・音価・Undo位置履歴を維持する。全体譜の小節選択で編集へ戻り、小節の選択欄でも直接移動できる。
  ...
  - 小節の詳細譜はその小節で鳴る区間を拡大表示する。小節線をまたぐタイの全体形状は全体譜で確認する。
  - 1小節2コードの編集パレットは引き続き最初のコードを使用（既知の制限）。
  - 自動回帰テスト: npm run test:entry（全編集STEPの初期データ、時刻保存、衝突、長音、3連変換、休符を検証）。
