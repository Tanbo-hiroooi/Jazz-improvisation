...
- **グリッドは「縦=音の高さ × 横=拍」のピアノロール**(GridEditor)。タップした交点がそのまま音になる。
  データモデル(`GridPhrase`)は1次元のままで、**変えたのは描き方と操作だけ**(保存済みフレーズもそのまま読める)。
  - 行 = `ROW_WINDOW`(G3〜D5 = 55〜74)内のパレット音 ∪ その小節で鳴っている音。`octShift`(左上の▲▼)で窓をオクターブ単位に動かす。
    範囲外でも「鳴っている音」の行は必ず残す(消えると音が編集できなくなる)。
  - `hold`は親attackのMIDIの行に描く(`soundingMidiMap`)。小節をまたぐと前の小節のコードのパレット外になりうるので、
    その行は`foreign`として表示のみ(置けない)。角丸は`run-end`(次がholdでない)だけに付けて1本の棒に見せる。
  - `fixedPitch`のSTEPは行を1本に畳む(第1章のリズム課題は従来と同じ見た目)。`fixedRhythm`は音のある位置だけ有効にして高さ変更を許す。
  - キーボードは roving tabindex + 矢印キー(セルが数百個になるのでタブ順には並べない)。Enter/Spaceで置く。
  - `engine.previewNote()`は専用シンセ。`melody`を使うと再生中の音を奪うため別ノートで、再生中は鳴らさない。
    ユーザー自身が置いた音を鳴らすだけなので見本演奏なし方針には抵触しない。
