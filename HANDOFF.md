...
最終更新: 2026-09-13（現在地と直近の変更一覧は AGENTS.md、履歴は git log を参照。2026-09-13 以降の修正は Codex が担当）
...
- **音符の順番入力**（GridEditor + theory/gridEntry.ts）:
  - 音価→音名で入力し、カーソルを自動で進める。音符タップは修正モード（自動で進めない）。休符ボタンは音価の列の末尾(選んだ長さで入る)。
  ...
  - **ドラッグ**(StaffView `onDragNote` → GridEditor): 最初に6px動いた向きで軸を固定。上下=`steps`(論理8pxごとにパレット1段)、
    ...
    ↑↓キー(`onNudgeNote`)は`stepPitch`で1段ずつ(▲▼ボタンは2026-09-13にオーナー指示で撤去)。
    「⇧/⇩ 1オクターブ」ボタンは1組だけ: 音を選んでいれば`shiftOctave`(その音を±12、パレットにある時だけ有効)、
    選んでいなければ`octave` state(音名ボタンの表示音域)を切り替える。旧「−1/+1オクターブ」は撤去。
    表情ボタンも常設: 選んでいればその音の`articulation`、選んでいなければ`pendingArtic`(次に入力する音に付く。音価と同じく保持)。
    「この音の次から入力」は撤去。修正モードから順番入力へ戻るのは、休符タップか「進む」。
...
- 表情記号は**符頭の側**に置く(`articlesToNoteheadSide`: 上向きの符幹なら下、下向きなら上)。連桁で符幹の向きが確定してから設定すること。
- 3連の数字は**符幹の側**(`setTupletLocation`: 上向き=上、下向き=下)。VexFlow 4の既定は「常に上」で、表情記号付きの3連や
  下向きの高い音で数字がコードネームの高さまで上がり、重なって見えた(2026-09-13 オーナー指摘)。
- コードネームは全小節・3連を描いた後に描く。その行の3連の数字(`g.vf-tuplet`)がコードネームの高さへ入るときは、
  行ごと持ち上げる安全策がある(1行目が切れないよう topPad は28)。検証は音域×譜表×表情記号の組み合わせを裏で描いて bbox を比べる。
